...
@CompileStatic
class BuildDockerAnalyticsGoPerformer {
    ...
    static void build(Environment imp, String path, VersionToBuild build, String imageName, boolean onlySource = false, Map<String, String> dockerBuildArgs = [:]) {
        ...
        imp.dirAbsolute(path) {
            imp.dir('transactions-fit-performer') {
                ...
                if (!onlySource) {
                    imp.execute("docker build -f performers/analytics/go/Dockerfile --platform=linux/amd64 $serializedBuildArgs -t $imageName .", false, true, true)
                }
            }
        }
    }
