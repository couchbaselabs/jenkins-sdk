...
class BuildDockerColumnarNodePerformer {
    ...
    static void build(Environment imp, String path, VersionToBuild build, String imageName, boolean onlySource = false) {
        ...
        imp.dirAbsolute(path) {
            imp.dir("transactions-fit-performer/performers/columnar/node") {
                writePackageFile(imp, build)
                TagProcessor.processTags(new File(imp.currentDir()), build)
            }
            if (!onlySource) {
                imp.log("building docker container")
                imp.execute("docker build -f transactions-fit-performer/performers/columnar/node/Dockerfile -t $imageName .", false, true, true)
            }
        }
    }
